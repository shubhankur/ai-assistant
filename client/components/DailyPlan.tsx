...
/* ------------------------ Time Helpers ------------------------------- */
const TZ = "America/New_York"; const MS30 = 1.8e6;
const hhmmToDate = (s: string)=>{const n=s.includes("+1");const [h,m]=s.replace("+1","").split(":").map(Number);const d=new Date();if(n)d.setDate(d.getDate()+1);d.setHours(h%24,m,0,0);return d;};
...
/* ------------------------ Functions ------------------------------- */
const onChange = (d:DayPlan) => {
    //update DB
}

/* ------------------------ Component ---------------------------------- */
interface Props{plan?:DayPlan}

export function DailyPlan(plan : DayPlan){
  // ensure every block has groupId
  const init:DayPlan={...plan,blocks:plan.blocks.map(b=>({...b,groupId:b.groupId??uuid()}))};
  const [state,setState] = useState<DayPlan>(init);
  const [editing,setEditing] = useState<number|null>(null);
  const inputRef = useRef<HTMLTextAreaElement|null>(null);
  useEffect(()=>{if(editing!==null) inputRef.current?.focus();},[editing]);

  /* build slots */
  const sorted=[...state.blocks].sort((a,b)=>hhmmToDate(a.start).getTime()-hhmmToDate(b.start).getTime());
  const firstActive = sorted.find(b=>b.category!=="sleep") ?? sorted[0];
  const start = hhmmToDate(firstActive.start);
  const firstSleep = sorted.find(b=>b.category==="sleep");
  const endTs = firstSleep ? hhmmToDate(firstSleep.start).getTime() + MS30*2 : hhmmToDate(sorted.at(-1)!.end).getTime();
  type Slot={time:string,block?:Block,idx?:number,slotStart:Date};
  const slots:Slot[]=[];
  for(let t=start.getTime(); t<endTs; t+=MS30){ const d=new Date(t); const idx=sorted.findIndex(b=>hhmmToDate(b.start).getTime()<=t && hhmmToDate(b.end).getTime()>t); const blk=idx>=0?sorted[idx]:undefined; slots.push({time:fmt(d), block: blk?.category==="sleep" ? {...blk, name:"Sleep"}:blk, idx: idx>=0?idx:undefined, slotStart:d}); }

  /* split helper */
  const splitBlock = (plan:DayPlan, idx:number, slotStart:Date):DayPlan => {
    const blk = plan.blocks[idx];
    const st = hhmmToDate(blk.start), ed = hhmmToDate(blk.end);
    if(ed.getTime()-st.getTime()===MS30) return plan;
    const slotEnd = new Date(slotStart.getTime()+MS30);
    const frags:Block[]=[];
    if(st < slotStart) frags.push({...blk, end: dateToHHMM(slotStart), groupId: blk.groupId});
    frags.push({...blk, start: dateToHHMM(slotStart), end: dateToHHMM(slotEnd, blk.end.includes("+1")), groupId: blk.groupId});
    if(slotEnd < ed) frags.push({...blk, start: dateToHHMM(slotEnd, blk.end.includes("+1")), end: blk.end, groupId: blk.groupId});
    return {...plan, blocks:[...plan.blocks.slice(0,idx), ...frags, ...plan.blocks.slice(idx+1)]};
  };

  /* handlers */
  const saveOne = (si:number) => {
    const sl = slots[si]; if(sl.idx===undefined || !inputRef.current) return;
    let next = splitBlock(state, sl.idx, sl.slotStart);
    const startKey = dateToHHMM(sl.slotStart);
    const bi = next.blocks.findIndex(b=>b.start===startKey);
    next.blocks[bi] = {...next.blocks[bi], name: inputRef.current.value.trim() || next.blocks[bi].name};
  ...
  const saveAll = (si:number) => {
    ...
    next.blocks = next.blocks.map(b=> b.groupId===gid ? {...b, name: inputRef.current!.value.trim() || b.name}:b);
    setState(next); onChange(next); setEditing(null);
  };
  ...
  return (
    <div className="max-w-xl mx-auto bg-gray-900 rounded-xl p-4 space-y-2 text-gray-100">
      <h2 className="text-lg font-semibold mb-2 select-none">{new Date(state.date).toLocaleDateString("en-US",{weekday:"long",month:"short",day:"numeric"})}</h2>
      {slots.map((s,i)=>(
