...
describe('User API', () => {
  ...
  it('gets user by email', async () => {
    const res = await request(app).get('/users').query({ email: 'a@test.com' });
...
describe('DailyPlan API', () => {
  ...
  it('gets plan by user', async () => {
    const res = await request(app).get('/dailyPlans').query({ userid: userId });
    if (!Array.isArray(res.body) || res.body[0]._id !== planId) throw new Error('Plan get by user failed');
  });

  it('gets plan by user and date', async () => {
    const res = await request(app).get('/dailyPlans').query({ userid: userId, date: '2024-01-01' });
    if (res.body._id !== planId) throw new Error('Plan get by user/date failed');
  });
