...
Rules
--> COVER ASPIRATIONS IN THIS ORDER
   a. goals
   b. lifestyle_changes
   c. activities_to_add
...
{{
  ...
  "LOW_PRIORITY": [
    ...
    "weekly_plan" : (
'''
Combine user current routine with their desired changes & our suggested changes to create their weekly routine.
...
Scheduling rules
    --> Explicit Rules. 
      - If the user specifided a specific day/time for an activity, schedule it exactly there.
      - Every activity must have a start time, end time, name, and category. For name and category, if unclear, use name as "open" and category as "other".
      - Since we are starting the day with wake up in the morning. Consider midnight, that is 00:00 or 12AM as the next day. So return it with +1.
      - Two activities cannot have the same start time or same end times.
      - Must maintain Chronological order inside each day after all inserts/merges.
    --> Sleep Block
      - For each day, there should always be one and only one block exactly with the category sleep and it should be the last acitivity of the day.
      - If the user has multiple sleep timings in a day, use category relax.
    --> Wake Up Block
      - For each day, there should always be one and only one block exactly with the category wakeup which demonstrates start of the day.
    --> User's Aspirations and Goals 
      - Based on the discussion with user about their aspirations, desired changes and goals, prepare the schedule that move the user from their current routine toward their aspirations.
      - Like a Licensed Occupational Therapist / Lifestyle Medicine Physician, include some activities that helps user have a better lifestyle and achieve their goals. For Example: Mindfulness, Enough Physical Activity, and Better Sleep.
    --> Break between continuous blocks:
